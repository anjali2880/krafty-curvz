...
@section('content')
<section
    class="relative flex items-center py-12 md:py-24 min-h-[300px] sm:min-h-[360px] md:min-h-[540px] bg-cover bg-center bg-no-repeat"
...
>
    ...
    <div class="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <h1 class="text-3xl sm:text-4xl font-bold text-white mb-3 md:mb-4">Contact {{ $siteSettings->site_name ?? 'Krafty Curvz' }}</h1>
        <p class="text-base md:text-lg text-white max-w-2xl mx-auto">Share your custom idea, gifting plan, or resin preservation request and our team will help you quickly.</p>
    </div>
</section>

<section class="py-10 md:py-20 bg-[#f5f2ed]">
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-5 md:gap-6">
            <div class="bg-white border border-amber-100 rounded-2xl shadow-soft p-5 md:p-7">
                <p class="text-xs uppercase tracking-[0.2em] text-amber-700 font-semibold">Email</p>
                @if(!empty($siteSettings->contact_email))
                    <a href="mailto:{{ $siteSettings->contact_email }}" class="mt-3 md:mt-4 inline-block text-base md:text-xl font-semibold text-neutral-900 hover:text-amber-700 transition-colors break-all">{{ $siteSettings->contact_email }}</a>
                @else
            ...
            <div class="bg-white border border-amber-100 rounded-2xl shadow-soft p-5 md:p-7">
                ...
                @if(!empty($siteSettings->contact_phone))
                    <a href="tel:{{ preg_replace('/\s+/', '', $siteSettings->contact_phone) }}" class="mt-3 md:mt-4 inline-block text-base md:text-xl font-semibold text-neutral-900 hover:text-amber-700 transition-colors">{{ $siteSettings->contact_phone }}</a>
                @else
            ...
            <div class="bg-white border border-amber-100 rounded-2xl shadow-soft p-5 md:p-7">
                ...
                @if(!empty($siteSettings->whatsapp_number))
                    <a href="https://wa.me/{{ preg_replace('/\D+/', '', $siteSettings->whatsapp_number) }}" target="_blank" rel="noopener noreferrer" class="mt-3 md:mt-4 inline-flex items-center text-base md:text-xl font-semibold text-green-700 hover:text-green-800 transition-colors">
        ...
        <div class="mt-6 md:mt-12 space-y-5 md:space-y-8">
            <div class="bg-white border border-amber-100 rounded-2xl shadow-soft p-5 md:p-8">
                <h2 class="text-2xl md:text-3xl font-bold text-[#5f3c2a]">Send Us a Message</h2>
                ...
                <form class="mt-5 md:mt-8 grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-5" action="#" method="POST">
                    <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-6">
                <div class="bg-white border border-amber-100 rounded-2xl shadow-soft p-5 md:p-7">
                    <h3 class="text-xl md:text-2xl font-bold text-[#5f3c2a]">Visit / Pickup Details</h3>
                ...
                <div class="bg-white border border-amber-100 rounded-2xl shadow-soft p-5 md:p-7">
                    <h3 class="text-xl md:text-2xl font-bold text-[#5f3c2a]">Need Quick Help?</h3>
