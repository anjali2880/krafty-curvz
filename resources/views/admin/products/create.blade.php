        ...
        <form method="POST" action="{{ route('admin.products.store') }}" enctype="multipart/form-data">
            ...
            <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <select name="category_id" id="category_id" required class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-amber-500 focus:border-amber-500">
                            ...
                            @foreach($categories->where('parent_id', null) as $mainCategory)
                                <option value="{{ $mainCategory->id }}" data-is-resin="{{ $mainCategory->slug === 'resin-products' ? '1' : '0' }}" {{ old('category_id') == $mainCategory->id ? 'selected' : '' }}>{{ $mainCategory->name }}</option>
                                @if($mainCategory->children->count() > 0)
                                    @foreach($mainCategory->children as $subcategory)
                                        <option value="{{ $subcategory->id }}" data-is-resin="{{ $mainCategory->slug === 'resin-products' ? '1' : '0' }}" {{ old('category_id') == $subcategory->id ? 'selected' : '' }}>-- {{ $subcategory->name }}</option>
                                    @endforeach
                                @endif
                            @endforeach
                        </select>
                ...
                <div>
                    ...
                    <textarea name="description" rows="4" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-amber-500 focus:border-amber-500">{{ old('description') }}</textarea>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Price *</label>
                        <input type="number" name="price" value="{{ old('price') }}" step="0.01" min="0" required class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-amber-500 focus:border-amber-500">
                        @error('price') <p class="text-red-500 text-sm mt-1">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Sale Price</label>
                        <input type="number" name="sale_price" value="{{ old('sale_price') }}" step="0.01" min="0" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-amber-500 focus:border-amber-500">
                ...
                <div>
                    ...
                    <input type="number" name="max_order_quantity" value="{{ old('max_order_quantity', 10) }}" min="1" max="999" required class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-amber-500 focus:border-amber-500">
                ...
                <div class="flex items-center">
                    <input type="checkbox" name="has_shape_options" id="has_shape_options" value="1" {{ old('has_shape_options') ? 'checked' : '' }} class="h-4 w-4 text-amber-600 focus:ring-amber-500 border-gray-300 rounded">
                    <label for="has_shape_options" class="ml-2 text-sm text-gray-700">Enable Shape Options</label>
                </div>

                <div id="shape-options-section" class="{{ old('has_shape_options') ? '' : 'hidden' }}">
                    ...
                    <div id="shape-options-container" class="space-y-2">
                        @php $oldShapes = old('shape_options', ['Round']); @endphp
                        @foreach($oldShapes as $index => $shape)
                ...
                <div class="flex items-center space-x-6">
                    <div class="flex items-center">
                        <input type="checkbox" name="customizable_product" id="customizable_product" value="1" {{ old('customizable_product') ? 'checked' : '' }} class="h-4 w-4 text-amber-600 focus:ring-amber-500 border-gray-300 rounded">
                        <label for="customizable_product" class="ml-2 text-sm text-gray-700">Customizable Product</label>
                    </div>
                </div>

                <div id="customizable-image-upload" class="{{ old('customizable_product') ? '' : 'hidden' }}">
                ...
                <div class="flex items-center space-x-6">
                    ...
                    <div class="flex items-center">
                        <input type="checkbox" name="is_featured" id="is_featured" value="1" {{ old('is_featured') ? 'checked' : '' }} class="h-4 w-4 text-amber-600 focus:ring-amber-500 border-gray-300 rounded">
                        <label for="is_featured" class="ml-2 text-sm text-gray-700">Featured</label>
                    </div>
                    <div class="flex items-center">
                        <input type="checkbox" name="manage_stock" id="manage_stock" value="1" {{ old('manage_stock') ? 'checked' : '' }} class="h-4 w-4 text-amber-600 focus:ring-amber-500 border-gray-300 rounded">
                ...
                <div class="flex justify-end space-x-3 pt-4">
                    ...
                    <button type="submit" class="px-4 py-2 bg-amber-700 hover:bg-amber-800 text-white rounded-lg text-sm font-medium">Create Product</button>
                </div>
            </div>
        </form>
    </div>
...
<script>
let sizeIndex = 1;
let shapeIndex = {{ count(old('shape_options', ['Round'])) }};
